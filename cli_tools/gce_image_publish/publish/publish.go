...
func (p *Publish) CreateWorkflows(ctx context.Context, varMap map[string]string, regex *regexp.Regexp, rollback, skipDup, replace, direct bool, oauth string) ([]*daisy.Workflow, error) {
	...
	for _, img := range p.Images {
		...
		w, err := p.createWorkflow(ctx, img, varMap, rollback, skipDup, replace, direct, oauth)
		if err != nil {
			return nil, err
		}
		if w == nil {
...
func publishImage(p *Publish, img *Image, pubImgs []*compute.Image, skipDuplicates, rep, direct bool) (*daisy.CreateImages, *daisy.DeprecateImages, *daisy.DeleteResources, error) {
...
func (p *Publish) populateWorkflow(ctx context.Context, w *daisy.Workflow, pubImgs []*compute.Image, img *Image, rb, sd, rep, direct bool) error {
	var err error
	...
	if rb {
		...
		createImages, deprecateImages, deleteResources, err = publishImage(p, img, pubImgs, sd, rep, direct)
		if err != nil {
			return err
		}
	}
...
func (p *Publish) createWorkflow(ctx context.Context, img *Image, varMap map[string]string, rb, sd, rep, direct bool, oauth string) (*daisy.Workflow, error) {
	...
	if err := p.populateWorkflow(ctx, w, pubImgs, img, rb, sd, rep, direct); err != nil {
